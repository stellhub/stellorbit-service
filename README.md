...
- 限流规则无法正交区分执行位置、全局协调方式和大模型 token/cost 限流。
...
- 应用进程、sidecar、网关、边缘代理等执行位置建模。
- 本地独立、每请求全局同步、异步配额租约等协调方式建模。
...
数据库中 `rate_limit_rules` 保存限流类型、算法、执行位置、全局协调方式、维度、配额、窗口、突发量、大模型配额指标和兜底策略。`execution_location` 表示规则在哪里执行，`coordination_mode` 表示是否需要全局同步或配额租约；分布式限流算法、配额分配、租约续期、热点分片和实际用量统计由独立限流运行时服务维护，Stellorbit Service 只把规则语义发布出去。

...
3. 操作，即 HTTP Method、读、写、删除、审批、发布、管理等动作。
...
不同位置适合不同粒度的规则。网关适合执行入口认证、Token 校验、租户入口限制、粗粒度 API 访问控制和统一审计。服务网格适合执行服务到服务的工作负载身份认证、命名空间隔离、服务账号授权、HTTP Method/Path 等协议层规则。应用服务适合执行业务资源级规则，例如资源归属、订单状态、项目成员关系、字段级权限、数据行级权限等。数据层适合执行最终的数据边界，例如行级安全策略、字段脱敏、表级权限和审计。
...
第三，操作属性。包括 HTTP Method、读、写、删除、审批、发布、管理、导出等动作。
...
Istio AuthorizationPolicy 的规则结构由 `from`、`to` 和 `when` 组成。`from` 表示请求来源，可基于 peer principal、request principal、namespace、service account、IP、trust domain 等字段匹配。`to` 表示请求操作，可基于 host、port、HTTP method、path 等字段匹配。`when` 表示附加条件，可基于 Istio 支持的属性进行匹配，例如请求 Header、source IP、remote IP、source namespace、source principal、JWT principal、JWT audience、JWT claims、destination IP、destination port、SNI 等 [R10][R13]。

因此，Istio 的鉴权规则既支持服务身份，也支持请求身份；既支持 L4 连接属性，也支持 L7 HTTP 属性；既支持允许和拒绝，也支持审计和外部授权扩展。
...
从粒度上看，规则至少应支持服务级、接口级和方法级。HTTP 场景中的方法级通常表现为 Method + Path，例如 `GET /orders/{id}`。Istio AuthorizationPolicy 已支持 HTTP Method、Path、Host、Port，因此服务网格层可以覆盖通用方法级访问控制 [R10]。
